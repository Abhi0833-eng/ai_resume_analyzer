...
function Results({ data, onReset }) {
  ...
  const downloadReport = () => {
    ...
    doc.text('ResumeRadar - Analysis Report', 20, 20)
    ...
    doc.text(`Overall Score: ${data.overallScore}/100`, 20, 40)
    doc.text(`Experience: ${data.experience}`, 20, 50)
    
    // ML Predictions
    if (mlData) {
      doc.setTextColor(147, 51, 234)
      doc.text('ML Model Predictions:', 20, 65)
      doc.setTextColor(0, 0, 0)
      doc.text(`ML Score: ${mlData.mlScore}`, 20, 75)
      doc.text(`Final Score: ${mlData.finalScore}`, 20, 85)
      doc.text(`Interview Chance: ${mlData.interviewProbability}%`, 20, 95)
      doc.text(`Selection Probability: ${mlData.selectionChance}`, 20, 105)
    }
    ...
    doc.setFontSize(12)
    doc.setTextColor(37, 99, 235)
    doc.text('Skills Detected:', 20, 150)
    doc.setTextColor(0, 0, 0)
    doc.setFontSize(10)
    const skillsText = data.skills.join(', ')
    const skillLines = doc.splitTextToSize(skillsText, 170)
    doc.text(skillLines, 20, 160)
    ...
    doc.setFontSize(14)
    doc.setTextColor(37, 99, 235)
    doc.text('AI Suggestions to Improve:', 20, 20)
    doc.setTextColor(0, 0, 0)
    doc.setFontSize(10)
    data.suggestions.forEach((s, i) => {
      const lines = doc.splitTextToSize(`${i + 1}. ${s}`, 170)
      doc.text(lines, 20, 35 + (i * 20))
    })
    
    // Footer
    doc.setFontSize(8)
    doc.setTextColor(150, 150, 150)
    doc.text('Generated by ResumeRadar - AI Powered Resume Analyzer', 20, 280)
    
    doc.save('ResumeRadar-Report.pdf')
  }
